...
// Use Native RBTC on RSK as collateralType for B-RAI.
contract RBTCJoin {
    // --- Auth ---
    mapping (address => uint256) public authorizedAccounts;
    /**
    ...
    **/
    modifier isAuthorized {
        require(authorizedAccounts[msg.sender] == 1, "RBTCJoin/account-not-authorized");
        _;
    }
    ...
    * @param account Account that will receive the RBTC representation inside the system
    **/
    function join(address account) external payable {
        require(contractEnabled == 1, "RBTCJoin/contract-not-enabled");
        require(int256(msg.value) >= 0, "RBTCJoin/overflow");
        safeEngine.modifyCollateralBalance(collateralType, account, int256(msg.value));
        emit Join(msg.sender, account, msg.value);
    }
    /**
    * @notice Exit rbtc from the system
    * @param account Account that will receive the rbtc representation inside the system
    **/
    function exit(address payable account, uint256 wad) external {
        require(int256(wad) >= 0, "rbtcJoin/overflow");
        safeEngine.modifyCollateralBalance(collateralType, msg.sender, -int256(wad));
        emit Exit(msg.sender, account, wad);
...
contract CoinJoin {
    ...
    /**
    * @notice Checks wthr msg.sender can call an authed function
    **/
    modifier isAuthorized {
